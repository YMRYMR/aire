...
- Recent progress: added direct provider-activation workflow tests, provider UI-state tests, chat-session tests, local API shaping tests, and tool-approval tests that cover provider persistence, conversation routing, cooldown messaging, token-usage presentation, message persistence, title updates, API snapshot normalization, approval policy fallback, and session-management behavior without the WPF shell.
