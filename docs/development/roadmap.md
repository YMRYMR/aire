...
- Recent progress: added direct provider-activation workflow tests, provider UI-state tests, chat-session tests, and local API shaping tests that cover provider persistence, conversation routing, cooldown messaging, token-usage presentation, message persistence, title updates, and API snapshot normalization without the WPF shell.
